...
**Counterfactual framing (relates to item 6, scale of decision):** advisor's July_17th comment frames the counterfactual as two things changing at once — (1) the scale at which the decision is made, (2) the identity of the deciding agent (two agents → one). **Model object now built (2026-07-21):** `model.tex` has a real `\subsection{Social Planner}` right after `\subsection{Equilibrium}`, using the same Lagrangian apparatus as §Transportation Authority (`eq:SP-problem`, extending `eq:TA-lagrangian` with a land-identity multiplier `ν_i` and letting `t_i^F`/`I_kl` be chosen jointly for the whole metro). Investment FOC is unchanged in form from `eq:I-optimal` (`eq:I-optimal-SP`), evaluated at the jointly-optimal zoning instead of the Nash one. The zoning FOC (`zoning-foc-planner`) adds two named, signed terms absent from the municipal FOC `zoning-foc-explicit`: (6) cross-jurisdiction spillover (planner sums `λ_i'j'` over all of `S`, not just `S^g`) and (7) transit-zoning complementarity (denser zoning raises `Λ_kl`, which raises optimal investment via `eq:I-optimal-SP` — a channel municipalities can't see since they take `I` as given under the simultaneous-Nash timing). Both weakly positive ⟹ `H̄_i^{R,SP}≥H̄_i^{R,DE}` for all `i`, derived (not asserted, unlike the old commented-out sketch this replaces). Still doesn't resolve item 1 (road/transit distinction). The *quantitative* counterfactual (solving this numerically, computing the welfare gap) is still future work — this is the model object only.
